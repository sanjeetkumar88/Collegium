...
function EventRSVPs() {
  ...
  const fetchEvents = async () => {
    try {
      setLoading(true);
      const response = await eventApi.getRsvps({ title: debouncedSearchTerm, page, limit });
  ...
  const handleEventClick = async (event) => {
    ...
    try {
      ...
      const response = await eventApi.getRegisteredUsers(event._id);
  ...
  return (
    <div className="relative min-h-screen bg-white overflow-hidden">
      ...
      >
        <Header 
          name="RSVPs" 
          title="Monitor guest lists and manage event attendance." 
          dashboard={true}
          request={true}
          yourEvent={true}
        />
        ...
        <div className="bg-white/40 backdrop-blur-xl p-8 rounded-[3rem] border border-slate-100 shadow-2xl">
          ...
          {loading && (
            <div className="flex justify-center py-20">
               <Loader text="Retrieving guest lists..." />
            </div>
          )}
