...
const SideBar = ({ isOpen, onClose, members = [], event, refetchMembers, refetchEvents }) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await eventApi.downloadWaitlistedUsers(event._id);
  ...
  const handleMemberAction = async (memberId, action) => {
    ...
    try {
      ...
      await eventApi.updateWaitlistStatus(event._id, memberId, { action });
  ...
  const handleDelete = async (memberId) => {
    try {
      ...
      await eventApi.removeEventMember(event._id, memberId);
