import {
  ...
  const EventSidebar = ({ title, category, date, location, meet, price, duration,onClick }) => {
    ...
    return (
      <aside className="w-full lg:w-1/3 bg-white rounded-xl shadow-lg p-6 space-y-4">
        ...
        <div className="flex items-center justify-between pt-2">
          ...
          <button className="bg-pink-600 hover:bg-pink-700 transition text-white px-4 py-2 rounded-lg font-semibold" onClick={onClick}>
            RSVP
          </button>
        </div>
      </aside>
