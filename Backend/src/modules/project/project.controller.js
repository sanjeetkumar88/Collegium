...
const joinProject = async (req, res) => {
  try {
    ...
    if(req.user.role === 'admin'){
      throw new ApiError(400,"Admin cannot join the project")
    }
    console.log(project.createdBy);
    

    if(project.createdBy === userId){
      throw new ApiError(400,"Author cannot join the project")
    }
...
const updateProjectCoverImage = async (req, res) => {
  try {
    const projectId = req.params.id;
...
const updateProjectDetails = async (req, res) => {
  try {
    ...
    throw new ApiError(500, error.message || "Something wents wrong")
  }
};
